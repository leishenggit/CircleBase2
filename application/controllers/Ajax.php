...
class Ajax extends CI_Controller {
    ...
    public function __construct(){
		...
		ini_set('max_execution_time', 2000);//秒为单位，自己根据需要定义
    }

	public function detail_mouse($eccDNA_id){
        ...
        $chr = $row['chr_mm10'];
        $start = $row['start_mm10'];
        $end = $row['end_mm10'];
        $data['region']  = $chr.":".$start.":".$end;
        $data['hgnc_symbol'] = $row['Gene_Name'];
        //get data for some tables
        $tables = array('Regulatory_element_dbSUPER_mouse', 'Regulatory_element_EnhancerAtlas_mouse', 'Regulatory_element_SEA_mouse', 'Regulatory_element_ChromHMM_mouse', 'Regulatory_element_cCRE_mouse',
		'Chromatin_access_ChIP_Atlas_ATAC_seq', 'Chromatin_access_ChIP_Atlas_DNase_seq','Epigenetic_regulation_ChIP_Atlas_TFBS','Epigenetic_regulation_ChIP_Atlas_HM');
        foreach($tables as $table){
            $sql = "select * from $table where chromosome='$chr' and start_position < $end and end_position > $start";
            $ciphertext = $this->encryption->encrypt($sql);
			$data[$table] = $ciphertext;
			//$data[$table] = $sql;
        }
        //genetic variants info
        $tables = array( 'dbSNP_mouse');
        foreach($tables as $table){
            $end_1 = $end-1;
        ...
        $this->load->view('detail_mouse', $data);
        $this->load->view('footer');
        exec("mkdir -p assets/circosJS/biocircos/$eccDNA_id");
	}
    ...
    public function table(){
        ...
        $sql = $this->encryption->decrypt($_POST['sql_line']);
        $table = $_POST['table'];
        $html_page = $table.".php";
    ...
    public function network(){
        ...
        exec("mkdir $dir");
        $sql = "select * from  network_2 where eccdna = ?";
        ...
        $data['gene'] = $genes;
		if(count($genes) > 0 ){
			$sql = "select * from network_2 where gene in ?";
    ...
    public function  enrichment(){
        ...
        if(!is_dir($dir)){
            ...
            fclose($myfile);
            //运行富集分析脚本
            exec("/Users/apple/miniforge3/envs/clusterprofiler/bin/Rscript assets/scripts/enricher.R eccdna_gene.txt $dir", $res, $code);
            if($code != 0){
    ...
    public function biocircos(){
        ...
        foreach($tables_para as $table){
            ...
            if( $code == 0 ){
				...
				$response['tables_para'] = $tables_para;
                $response['info'] = "wc -l exec failed";
                exit(json_encode($response));
            }
        }
    ...
    public function eccdna_score(){
        $eccdna = $_POST['eccDNA'];
		if(preg_match('/^mm10_/', $eccdna))
			$sql = "select * from  hits_stat_mouse where eccdna = ?";
		else
			$sql = "select * from  hits_stat_v2 where eccdna = ?";
        $query = $this->db->query($sql, array($eccdna)); #search the eccdna
        foreach ($query->result() as $row){
            $hits[] = $row;
        }
		if(preg_match('/^mm10_/', $eccdna))
			$sql = "select * from  eccDNA_score_mouse where eccdna = ?";
		else
			$sql = "select * from  eccDNA_score_v2 where eccdna = ?";
        $query = $this->db->query($sql, array($eccdna)); #search the eccdna
        foreach ($query->result() as $row){
            $score[] = $row;
        }
	...
	public function motif($url_1){
		$parts = explode("_js1_", $url_1);
		$data['url_1'] = "assets/motif/".$parts[0]."/js1.bed.MotifOutput/knownResults.html";
		$data['url_2'] = "assets/motif/".$parts[0]."/js2.bed.MotifOutput/knownResults.html";
		$this->load->view('header');
		$this->load->view('motif', $data);
		$this->load->view('footer');
	}
}
